...
const shouldUpdateLeft = (leftOldNode, leftNewNode) =>
  !isTextNode(leftOldNode) && matchNodeType(leftOldNode, leftNewNode);

const shouldUpdateRight = (rightOldNode, rightNewNode) =>
  !isTextNode(rightOldNode) && matchNodeType(rightOldNode, rightNewNode);

const decideUpdateDirection = (nodes, updates) => {
  const { leftOldNode, leftNewNode, rightOldNode, rightNewNode } = nodes;
  const { updateLeft, updateRight } = updates;
  ...
  if (updateLeft && updateRight) {
    ...
    if (equalityLeft < equalityRight) {
      return { updateLeft: false, updateRight: true };
    }
    return { updateLeft: true, updateRight: false };
  }
  return { updateLeft, updateRight };
};

const processNodePair = (params) => {
  const {
    schema,
    leftOldNode,
    leftNewNode,
  ...
  if (Array.isArray(leftOldNode) && Array.isArray(leftNewNode)) {
    ...
    return { type: "text-array", incrementLeft: 1, incrementRight: 0 };
  }

  let updateLeft = shouldUpdateLeft(leftOldNode, leftNewNode);
  let updateRight = shouldUpdateRight(rightOldNode, rightNewNode);

  const decision = decideUpdateDirection(
    { leftOldNode, leftNewNode, rightOldNode, rightNewNode },
    { updateLeft, updateRight }
  );
  updateLeft = decision.updateLeft;
  updateRight = decision.updateRight;

  if (updateLeft) {
    finalLeftChildren.push(patchDocumentNode(schema, leftOldNode, leftNewNode));
    return { type: "update-left", incrementLeft: 1, incrementRight: 0 };
  }
  ...
  if (updateRight) {
    ...
    return { type: "update-right", incrementLeft: 0, incrementRight: 1 };
  }
  ...
  return { type: "delete-insert", incrementLeft: 1, incrementRight: 0 };
};
...
const patchRemainNodes = (schema, oldChildren, newChildren) => {
  ...
  while (oldChildLen - left - right > 0 && newChildLen - left - right > 0) {
    ...
    left += result.incrementLeft;
    right += result.incrementRight;
  }
...
const areArrayNodesEqual = (node1, node2) =>
  node1.length === node2.length &&
  node1.every((node, index) => isNodeEqual(node, node2[index]));

const areAttributesEqual = (attrs1, attrs2) => {
  const attrs = [...new Set([...Object.keys(attrs1), ...Object.keys(attrs2)])];
  return attrs.every((attr) => attrs1[attr] === attrs2[attr]);
};
...
const areMarksEqual = (marks1, marks2) => {
  ...
  return marks1.every((mark, i) => isNodeEqual(mark, marks2[i]));
};

const areChildrenEqual = (children1, children2) => {
  if (children1.length !== children2.length) {
    return false;
  }
  return children1.every((child, i) => isNodeEqual(child, children2[i]));
};
...
export const isNodeEqual = (node1, node2) => {
  ...
  if (isNode1Array) {
    return areArrayNodesEqual(node1, node2);
  }
  ...
  if (!areAttributesEqual(attrs1, attrs2)) {
    return false;
  }
  ...
  return areChildrenEqual(children1, children2);
};
...
export const normalizeNodeContent = (node) => {
  ...
  for (let i = 0; i < content.length; i++) {
    ...
    if (isTextNode(child)) {
      ...
      while (i < content.length && isTextNode(textNode)) {
